...
namespace NAudio.Midi 
{
    ...
    public class MidiFile 
    {
        ...
        public MidiFile(string filename, bool strictChecking)
        {
            ...
            ConvertBinaryToMidi(br);
        }

        public MidiFile(BinaryReader binary, bool strictChecking = true)
        {
            this.strictChecking = strictChecking;

            ConvertBinaryToMidi(binary);
        }

        private void ConvertBinaryToMidi(BinaryReader binary) 
        {
            //using (binary)
            {
                string chunkHeader = Encoding.UTF8.GetString(binary.ReadBytes(4));
                if (chunkHeader != "MThd")
                {
                    throw new FormatException("Not a MIDI file - header chunk missing");
                }
                uint chunkSize = SwapUInt32(binary.ReadUInt32());
                ...
                fileFormat = SwapUInt16(binary.ReadUInt16());
                int tracks = SwapUInt16(binary.ReadUInt16());
                deltaTicksPerQuarterNote = SwapUInt16(binary.ReadUInt16());
                ...
                for (int track = 0; track < tracks; track++)
                {
                    ...
                    chunkHeader = Encoding.UTF8.GetString(binary.ReadBytes(4));
                    if (chunkHeader != "MTrk")
                    {
                        throw new FormatException("Invalid chunk header");
                    }
                    chunkSize = SwapUInt32(binary.ReadUInt32());

                    long startPos = binary.BaseStream.Position;
                    MidiEvent me = null;
                    var outstandingNoteOns = new List<NoteOnEvent>();
                    while (binary.BaseStream.Position < startPos + chunkSize)
                    {
                        me = MidiEvent.ReadNextEvent(binary, me);
                        ...
                        else if (me.CommandCode == MidiCommandCode.MetaEvent)
                        {
                            ...
                            if (metaEvent.MetaEventType == MetaEventType.EndTrack)
                            {
                                ...
                                if (strictChecking)
                                {
                                    if (binary.BaseStream.Position < startPos + chunkSize)
                                    {
                    ...
                    if (binary.BaseStream.Position != startPos + chunkSize)
                    {
                        throw new FormatException(String.Format("Read too far {0}+{1}!={2}", chunkSize, startPos, binary.BaseStream.Position));
                    }
                }
            }
        }
